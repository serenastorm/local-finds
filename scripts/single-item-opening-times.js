...
function renderLocationStatus(_a) {
  var locationName = _a.locationName,
    hours = _a.hours;
  var openingSoonStatus = document.querySelectorAll(
    ".status.opening-soon[data-location='".concat(locationName, "']")
  );
  var closingSoonStatus = document.querySelectorAll(
    ".status.closing-soon[data-location='".concat(locationName, "']")
  );
  var openStatus = document.querySelectorAll(
    ".status.open[data-location='".concat(locationName, "']")
  );
  var closedStatus = document.querySelectorAll(
    ".status.closed[data-location='".concat(locationName, "']")
  );
  var renderStatus = function (status) {
    if (status === "OPEN") {
      openStatus.forEach(function (el) {
        return (el.style.display = "flex");
      });
    }
    if (status === "CLOSED") {
      closedStatus.forEach(function (el) {
        return (el.style.display = "flex");
      });
    }
    if (status === "CLOSING_SOON") {
      closingSoonStatus.forEach(function (el) {
        return (el.style.display = "flex");
      });
    }
    if (status === "OPENING_SOON") {
      openingSoonStatus.forEach(function (el) {
        return (el.style.display = "flex");
      });
    }
  };
  var getWeekDaysWithOpeningTimes = function () {
    ...
    orderedWeekDays.forEach(function (weekDay) {
      var weekDayRegex = new RegExp(
        "(".concat(
          weekDay,
          ")\\s([01]?[0-9]|2[0-3]):([0-5][0-9])-([01]?[0-9]|2[0-3]):([0-5][0-9])"
        )
      );
      var weekDayRegexMatches = hours.match(weekDayRegex);
      var hoursCanBeParsed =
        (weekDayRegexMatches === null || weekDayRegexMatches === void 0
          ? void 0
      ...
      if (hoursCanBeParsed) {
        ...
        var currentWeekDayIndex = jsWeekDays.findIndex(function (jsWeekDay) {
          return jsWeekDay === weekDay;
        });
  ...
  var getStatus = function () {
    ...
    weekDaysWithOpeningTimes.forEach(function (weekDayWithOpeningTimes) {
      if (weekDayWithOpeningTimes.isOpen) {
        var openingTime = weekDayWithOpeningTimes.openingTime,
          closingTime = weekDayWithOpeningTimes.closingTime;
...
window.onload = function () {
  var initialLocations = document.querySelectorAll(
